...
从颜色插值，显示到访问纹理数据，内建的`z-depth`可见性检查，越来越多的图形加速特性被添加到硬件中。由于这些图形加速特性的频繁使用，提出了专用的硬件(`GPU`)来提升性能,同时也在GPU的后代发展中添加了更多的绘制管线特性。专用的加速图形硬件和CPU相比的优势仅在于速度，而速度对于图形处理来说是至关重要的。
...
不同的处理器架构采用不同的策略来避免阻塞。CPU被优化于处理种类广泛的数据结构和大规模的代码。尽管CPU可以有多个处理器，但每个处理器几乎是串行地运行代码，大大限制了`SIMD`的能力。为了避免阻塞，CPU包含快速局部缓存，保证数据访问高命中，同时也采用了一些更智能的策略，如指令重排序，寄存器重命名，缓存预获取等。
...
GPU主要为流量(`throughput`)进行优化，来尽可能快的处理更多数据。然而，由于更小的芯片面积限制，单个GPU的缓存和控制逻辑能力比CPU弱的多，因而对每个着色器核心来说，延迟问题也更加突出。
...
更进一步，GPU把指令执行逻辑从数据中分离开，以一致的步伐执行固定数量的着色程序，也就是所谓的SIMD。此外，执行相同着色程序的GPU线程被捆成一束(`warps`)，被一定数量的GPU着色器核心使用SIMD处理的方式来进行调度。假设有两千个线程需要执行，Nvidia中的GPU中的`warps`包含32个线程，那个就会产生2000/30=62.5个`warps`。一个`warps`对一组相同数量(32个)的顶点或像素运行相同的着色程序，当遇到耗时操作发生阻塞时，`warps`就会切换到下一组顶点或像素进行处理。这要求一个`warps`的32个线程尽可能的同时处理完所有的顶点或像素，否则就会发生一个`warps`间的线程等待耗时问题。
...
底层的虚拟机给不同类型的输入输出提供不同的寄存器。为`uniform`提供的固定寄存器`constant register`的大小远大于为可变的输入输出提供的寄存器。这是因为，每一个顶点或像素都需要单独存储可变的输入输出，这天然限制了这些寄存器的大小。而`uniform`输入在每一次`draw call`中只需要存储一次，并且为所有顶点或像素共享。此外，还有通用目的的临时寄存器`temporary register`。
